...
class image_converter:
  def __init__(self):
    ...
    # Phase 6
    self.ref_phase6_speed_F = self.ref_speed_F + 5
    self.ref_phase6_speed_B = self.ref_speed_B + 5
    # Phase 7
    self.ref_phase7_speed_F = self.ref_speed_F + 5
    ...
    self.Phase6_B_KE = -4
    # Phase 7
    self.Phase7_F_KX = 0.2
    self.Phase7_F_KE = 1
    self.Phase7_B_KX = 0.2
    self.Phase7_B_KE = 1
    ...
    # PHase 5
    self.Phase5_tolerance1 = 0.45 # forward, 90cm
    self.Phase5_tolerance2 = 0.87 # backward, 40cm
    # Phase 6
    self.Phase6_tolerance1 = 17 # 진입할때의 tolerance : 진입한지 어느정도 지나고 yaw_error 가 클 때의 tolerance
    ...
    self.Phase6_tolerance4 = 1.05
    # Phase 7
    self.Phase7_tolerance1 = 20 # 마지막 주차할때 Obstacle과의 거리(이거보다 작아야함)
    self.Phase7_tolerance2 = 408 # 마지막 주차할때 Obstacle과의 거리(이거보다 커야함)
    # Phase 10
    self.Phase10_tolerance1 = 3 # 뒤로 Back 했을 떄 어느정도 갈지에 대한 tolerance
    self.Phase10_tolerance2 = 0.3
...
###########################################################################################
  ...
  def line_obstacle_detector(self):
    ...
    # for line in [left_point]: 
    #   img_t = cv2.line(img_pered_left, tuple(line[:2]), tuple(line[2:]), (255,0,0),4)
    #   cv2.imwrite('img_pered_left.jpg',img_t)
...
###########################################################################################
  ...
  def AutonomousParking(self, var1, var2, var3, var4, var5, var6):
    ...
    ## validation
      ...
      # Phase 6 -> 7 constraint
      if self.direction == 1: # 전방주차일 때,
        if time_diff > 1 and abs(yaw_error) < self.Phase6_tolerance1 and var2 is not None: # 조금만 있다가(to be robust), yaw error가 적어져서 드갈 수 있다고 판단되면,
          self.Phase6 = 0
          self.Phase7 = 1
          di = 93
          output_velocity = 0
        ...
        if time_diff > 1 and abs(yaw_error) < self.Phase6_tolerance1 and var3 is not None: # 조금만 있다가(to be robust), yaw error가 적어져서 드갈 수 있다고 판단되면,
          self.Phase6 = 0
          self.Phase7 = 1
          di = 93
          output_velocity = 0
      ...
      di = self.feedback_control(yaw_error,x_error)
      if di >= self.di_max - 30:
        di = self.di_max - 30
      elif di <= self.di_min + 30:
        di = self.di_min + 30
      di = di + 90
      output_steering = di
